...
double* getFileMatrix(std::string path, int row, int col) {
  double* matrix = new double[row * col]();
  ifstream f(path);
  for (int i = 0; i < row; i++)
    for (int j = 0; j < col; j++) f >> matrix[i * col + j];
  return matrix;
}

void writeToFile(double* X, string name) {
...
int main(int argc, char* argv[]) {
  ...
  double *A, *B, *C, *A_block, *B_block, *C_block, *C1;
  ...
  if (coords[X] == 0 && coords[Y] == 0) {
    ...
    B = generateMatix(n_2, n_3);
    writeToFile(A, "mat_a.txt");
    writeToFile(B, "mat_b.txt");
    A = getFileMatrix("mat_a.txt", n_1, n_2);
    B = getFileMatrix("mat_b.txt", n_2, n_3);

    double* C1 = new double[n_1 * n_3];
    simple_multiply(A, B, n_1, n_2, n_3, C1);
    writeToFile(C1, "mat_c1.txt");
  ...
  if (processRank == 0) {
    ...
    writeToFile(C, "mat_c_parallel.txt");
  }
